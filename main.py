...
import time
import sys
from sent_embed import sent_featurize
        ...
        for class_imb_flag in conf_dict["class_imb_flags"]:
            ...
            for model_type in conf_dict["model_types"]:
                for word_feats_raw in conf_dict["word_feats_l"]:
                    ...
                    for sent_feats_raw in conf_dict["sent_feats_l"]:
                        sent_feats, sent_feat_str = sent_featurize(sent_feats_raw, data_dict, conf_dict_com['poss_sent_feats_emb_dict'], conf_dict_com['use_saved_sent_feats'], conf_dict_com['save_sent_feats'], conf_dict_com["data_folder_name"], conf_dict_com["save_folder_name"], conf_dict_com["test_mode"])
                                            ...
                                            for att_dim in conf_dict["att_dims"]:
                                                for stack_rnn_flag in conf_dict["stack_rnn_flags"]:
                                                    ...
                                                    for thresh in conf_dict["threshes"]:
                                                        startTime = time.time()
                                                        info_str = "model: %s, word_feats = %s, sent_feats = %s, prob_trans_type = %s, class_imb_flag = %s, num_cnn_filters = %s, cnn_kernel_set = %s, rnn_type = %s, rnn_dim = %s, att_dim = %s, max_pool_k_val = %s, stack_rnn_flag = %s, thresh = %s, test mode = %s" % (model_type,word_feat_str,sent_feat_str,prob_trans_type,class_imb_flag,num_cnn_filters,cnn_kernel_set,rnn_type,rnn_dim,att_dim,max_pool_k_val,stack_rnn_flag, thresh, conf_dict_com["test_mode"])
                                                        fname_part = ("%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s~%s" % (model_type,word_feat_str,sent_feat_str,prob_trans_type,class_imb_flag,num_cnn_filters,cnn_kernel_set_str,rnn_type,rnn_dim,att_dim,max_pool_k_val,stack_rnn_flag, conf_dict_com["test_mode"]))
